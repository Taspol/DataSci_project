...
def extract_coredata(coredata):
    ...
    coredata_flat['creator_given_name'] = creator_info.get('ce:given-name', 'N/A')
    coredata_flat['creator_surname'] = creator_info.get('ce:surname', 'N/A')
    coredata_flat['creator_auid'] = creator_info.get('@auid', 'N/A')

    # Handle `link` field
    coredata_flat['link_self'] = next((link['@href'] for link in coredata.get('link', []) if link['@rel'] == 'self'), 'N/A')
    coredata_flat['link_scopus'] = next((link['@href'] for link in coredata.get('link', []) if link['@rel'] == 'scopus'), 'N/A')
...
def get_keywords(record_data):
    ...
    auth_keywords = record_data.get("abstracts-retrieval-response", {}).get("authkeywords", {})
    if auth_keywords:
...
def extract_funding_agencies(funding_list):
    ...
    for funding in ensure_list(funding_list):
        funding_agency = funding.get("xocs:funding-agency-matched-string", None)
        if funding_agency:
...
def process_record(file_path):
    ...
    try:
        ...
        author_group = record_data.get('abstracts-retrieval-response', {}).get('item', {}).get('bibrecord', {}).get("head", {}).get("author-group", [])
        funding_list = record_data.get('abstracts-retrieval-response', {}).get('item', {}).get('xocs:meta', {}).get("xocs:funding-list", {}).get("xocs:funding", [])
